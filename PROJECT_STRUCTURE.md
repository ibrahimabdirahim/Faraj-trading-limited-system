...
│   └── dev.db           The SQLite database file itself (git-ignored)
└── backup/              Timestamped dev.db snapshots taken before risky migrations
```

The whole system's data is the single file `database/prisma/dev.db` — copy it to back
up everything, restore by putting it back.
