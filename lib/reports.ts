...
import { getBranchComparison, startOfToday, dayStart, addDays } from "./metrics";
...
async function branchSummaryTable(opts: { slug: string; title: string; subtitle: string; from: Date; to: Date; filters: ReportFilters }): Promise<ReportTable> {
  ...
  return { slug, title, subtitle, generatedAt: new Date(), columns, rows, totals, numericCols: [2, 3, 4, 5, 6] };
}

async function dailyTable(filters: ReportFilters): Promise<ReportTable> {
...
async function customTable(filters: ReportFilters): Promise<ReportTable> {
  const from = filters.from ?? startOfToday();
  const to = filters.to ?? addDays(from, 1);
  return branchSummaryTable({ slug: "custom", title: "Custom Date Report", subtitle: rangeLabel(from, to), from, to, filters });
}

async function monthlyTable(filters: ReportFilters): Promise<ReportTable> {
...
async function financeTable(filters: ReportFilters): Promise<ReportTable> {
  const from = filters.from ?? startOfToday();
  const to = filters.to ?? addDays(from, 1);
...
async function weeklyTable(filters: ReportFilters): Promise<ReportTable> {
  ...
  return { slug: "weekly", title: "Weekly Report", subtitle: `Week ending ${fmtDate(endDay)}`, generatedAt: new Date(), columns, rows, totals: ["Total", tCdf, tUsd, tExpCdf, tExpUsd], numericCols: [1, 2, 3, 4] };
}

async function branchTable(filters: ReportFilters): Promise<ReportTable> {
  const from = filters.from ?? startOfToday();
  const to = filters.to ?? addDays(from, 1);
  ...
  totals.push("");
  return { slug: "branch", title: "Branch Report", subtitle: `Ranked by performance — ${rangeLabel(from, to)}`, generatedAt: new Date(), columns, rows: tableRows, totals, numericCols: [0, 3, 4, 5, 6, 7] };
}

async function inventoryTable(filters: ReportFilters): Promise<ReportTable> {
  const branches = filters.branchId
    ? await prisma.branch.findMany({ where: { id: filters.branchId } })
    : await prisma.branch.findMany({ where: { type: "branch", active: true }, orderBy: { sortOrder: "asc" } });
...
async function expenseTable(filters: ReportFilters): Promise<ReportTable> {
  const from = filters.from ?? startOfToday();
  const to = filters.to ?? addDays(from, 1);
  ...
  return { slug: "expense", title: "Expense Report", subtitle: `By category — ${rangeLabel(from, to)}`, generatedAt: new Date(), columns, rows, totals, numericCols: [1, 2] };
}

async function profitTable(filters: ReportFilters): Promise<ReportTable> {
  const from = filters.from ?? startOfToday();
  const to = filters.to ?? addDays(from, 1);
  let rows = await getBranchComparison(from, to);
  if (filters.branchId) rows = rows.filter((r) => r.id === filters.branchId);
  ...
  return { slug: "profit", title: "Profit Report", subtitle: `Margins & net profit — ${rangeLabel(from, to)}`, generatedAt: new Date(), columns, rows: tableRows, totals, numericCols: [0, 2, 3, 4] };
}
...
async function purchaseTable(filters: ReportFilters): Promise<ReportTable> {
...
async function paymentTable(filters: ReportFilters): Promise<ReportTable> {
  ...
  const columns = ["Date", "Supplier", "Amount", "Currency", "Method", "Reference", "Recorded By"];
  const rows = payments.map((p) => [fmtDate(p.date), p.supplier.name, p.amount, p.currency, p.method, p.referenceNumber || "—", p.createdBy?.name ?? "—"]);
  const totalsByCurrency: Record<string, number> = {};
  for (const p of payments) totalsByCurrency[p.currency] = (totalsByCurrency[p.currency] ?? 0) + p.amount;
  const totalsLabel = Object.entries(totalsByCurrency).map(([cur, amt]) => `${cur} ${amt.toLocaleString()}`).join(" · ") || "—";
  return { slug: "payment", title: "Payment Report", subtitle: `Supplier payments — ${filters.from && filters.to ? rangeLabel(filters.from, filters.to) : "all time"}`, generatedAt: new Date(), columns, rows, totals: ["Total", "", totalsLabel, "", "", "", ""], numericCols: [2] };
}

async function goodsReceivedTable(filters: ReportFilters): Promise<ReportTable> {
...
async function outstandingBalanceTable(filters: ReportFilters): Promise<ReportTable> {
  ...
  const rows = suppliers.map((s) => {
    ...
    for (const p of s.payments) { if (p.currency === "USD") payUsd += p.amount; else payCdf += p.amount; }
...
export async function getReportTable(slug: string, filters: ReportFilters = {}): Promise<ReportTable> {
  ...
  return applyCurrencyFilter(table, filters.currency);
}
