import { getDashboard, getBranchComparison, getProductsReceivedToday, getTodayExpenseCount, getOverallCashCollected, getAvailableCash, resolveDateRange } from "@/lib/metrics";
...
export default async function DashboardPage() {
  ...
  const [{ submitted, totalBranches, totals, activity }, settings, comparisonRows, productsReceived, expenseCount, overall] = await Promise.all([
    getDashboard(),
    getSettings(),
  ...
  const availableCash = await getAvailableCash(totals, overall);
  ...
  return (
    <>
      ...
      <TodaySummaryGrid submitted={submitted} totalBranches={totalBranches} totals={totals} expenseCount={expenseCount} productsReceived={productsReceived} />
