...
export default function ValuationForm({ branches }: { branches: { id: string; name: string }[] }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [f, setF] = useState({ branchId: branches[0]?.id ?? "", periodType: "weekly", periodEnding: today(), valueCdf: "", valueUsd: "", remarks: "" });
  ...
  return (
    <>
      <button className="btn btn-primary" onClick={() => setOpen(true)}><Icon name="plus" className="ico" size={16} />Enter inventory value</button>
      {open && (
