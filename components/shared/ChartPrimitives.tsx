...
export function Donut({ data, size = 150, centerTop, centerSub }: { data: { v: number; color: string }[]; size?: number; centerTop: string; centerSub: string }) {
  ...
  const lengths = data.map((d) => (d.v / tot) * C);
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size}>
      ...
      {data.map((d, i) => {
        ...
        return <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={d.color} strokeWidth="16" strokeDasharray={`${len} ${C - len}`} strokeDashoffset={-off} transform={`rotate(-90 ${cx} ${cy})`} />;
      })}
